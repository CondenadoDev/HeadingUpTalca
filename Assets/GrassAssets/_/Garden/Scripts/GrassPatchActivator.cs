    ...
    public class GrassPatchActivator : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (_isSetUp)
            {
                ...
                try
                {
                    Texture tex = _mainMaterial.GetTexture(_txid);
                    ...
                    {
                        Debug.LogWarning($"Texture for _NoGrassTex is not a RenderTexture. Using fallback texture.");
                ...
                {
                    Debug.LogWarning($"Failed to retrieve RenderTexture: {ex.Message}. Using fallback texture.");
                    renderTexture = new RenderTexture(512, 512, 0);
                    renderTexture.Create();
                }
